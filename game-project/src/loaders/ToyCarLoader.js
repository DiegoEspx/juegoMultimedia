...
export default class ToyCarLoader {
    ...
    async loadFromAPI() {
        try {
            ...
            try {
                const apiUrl =
                    import.meta.env.VITE_API_URL + '/api/blocks';
    ...
    _processBlocks(blocks, precisePhysicsModels) {
        // ✅ Reiniciar premios para este nivel
        this.prizes = [];
        ...
        blocks.forEach(block => {
            ...
            if (!glb) {
                console.warn(`Modelo no encontrado: ${block.name}`);
                return;
            }
            ...
            // Limpieza de cámaras/luces embebidas
            model.traverse((child) => {
                if (child.isCamera || child.isLight) child.parent.remove(child);
            });
            ...
            if (roleRaw === 'coin' || roleRaw === 'finalprize') {
                ...
                const prize = new Prize({
                    model,
                    position: new THREE.Vector3(block.x || 0, block.y || 0, block.z || 0),
                    scene: this.scene,
                    role: roleNorm
                });
                ...
                return; // ⬅️ No añadir físicas ni modelo a escena (ya lo hace Prize)
            }


            // 🚧 Escena + físicas para objetos estáticos
            this.scene.add(model);

            let shape, position = new THREE.Vector3();
        ...
        const coins = (this.prizes || []).filter(p => p.role === 'coin');
        this.requiredCoins = coins.length;


        // Ocultar el portal al inicio
        /* const final = (this.prizes || []).find(p => p.role === 'finalPrize');
        if (final?.pivot) {
            final.pivot.visible = false;
            if (final.model) final.model.visible = false;
        } */
    }
}