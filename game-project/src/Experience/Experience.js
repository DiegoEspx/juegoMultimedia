...
export default class Experience {
  ...
  update(delta) {
    // Cámara (solo en no-VR y si no estás en 3ra persona)
    if (!this.isThirdPerson && !this.renderer.instance.xr.isPresenting) {
      this.camera.update()
    }

    // Ajuste de cámara para VR
    if (this.renderer.instance.xr.isPresenting) {
      this.adjustCameraForVR()
    }

    // 1) 👉 Primero físicas (aplica fuerzas y avanza cuerpos)
    this.physics.update(delta)

    // 2) 👉 Luego lógica del mundo (lee posiciones de física y actualiza escena)
